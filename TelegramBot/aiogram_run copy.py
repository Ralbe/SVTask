...
async def check_ad_filters(ad: tuple, filters: dict) -> bool:
    ...
    ad_price = ad[5]
    # try:
    #     ad_price = int(ad[5].replace('₽', '').strip())
    # except (ValueError, AttributeError):
    #     ad_price = 0
    
    # Проверяем фильтры
    # if 'ad_id' in filters and ad_id != int(filters['ad_id']):
    #     return False
    # if 'author' in filters and ad_author != int(filters['author']):
    #     return False
...
async def find_next_filtered_ad(current_index: int, filters: dict, forward: bool = True) -> int:
    ...
    while checked < len(ads):
        ...
        if await check_ad_filters(ads[current_index], filters):
...
async def display_current_ad(message: Message, state: FSMContext):
    ...
    filters = {
        ...
        'ad_id':data.get('ad_id','не указан')
    }
    
    # Проверяем текущее объявление на соответствие фильтрам
    if not await check_ad_filters(ads[current_index], filters):
        # Если текущее не подходит, ищем следующее подходящее
        current_index = await find_next_filtered_ad(current_index, filters, forward=True)
...
async def next_ad(message: Message, state: FSMContext):
    ...
    new_index = await find_next_filtered_ad(current_index, filters, forward=True)
...
async def prev_ad(message: Message, state: FSMContext):
    ...
    new_index = await find_next_filtered_ad(current_index, filters, forward=False)
...
# @dp.message(F.text == "Сохраненные объявления", state = ViewingAds)
# async def show_ads(message: Message, state: FSMContext):
#     await message.answer("Выберите интересующий вариант.", reply_markup=types.ReplyKeyboardRemove())
#     await state.update_data(viewing_ad=0)  # Инициализируем индекс
#     await state.update_data(saved_ads = get_saved_by_user(get_user_id_by_tg_id(message.from_user.id)))
#     await state.set_state(ViewingOwnAds.viewing_ad)
#     await display_current_ad(message, state)  # Показываем первое объявление
